...
def main():
    ...
    raw_crispr_data = raw_crispr_data[
        raw_crispr_data.index.isin(rna_seq_lines["profiled_lines"])
    ]
    ...
    filtered_crispr_data["Gene"] = [
        x.split(" ")[0] for x in filtered_crispr_data["Gene"].values.tolist()
    ]

    ## Resolve duplicates keeping most variable gene
    filtered_crispr_data_uniques = (
        filtered_crispr_data.sort_values("variance", ascending=False)
    ...
    # melt the dataset
    melted_crispr = filtered_crispr_data_uniques.melt(
        id_vars=["Gene"], var_name="cell_line", value_name="probability"
    )
    ...
    for gene_model in melted_by_gene.groups.items():
        ...
        model_data.to_csv(
            f"{where_to_save}/{gene_name}.csv", sep=",", index=False, header=True
        )
