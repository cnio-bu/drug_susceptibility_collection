...
generate_bidirectional_signature <- function(sig_name, deg_genes){
    ...
    if(length(dependency_upregulated) >= 5){
       ...
       setName(sensitivity_gset) <- paste(sig_name, "UP", sep="_")
        ...
        GSEABase::toGmt(sensitivity_gset, con = paste(geneset_directory, "/", sig_name, "_met_UP", ".gmt", sep=""))
    }
    if(length(dependency_downregulated) >= 5){
        ...
        GSEABase::toGmt(resistance_set, con = paste(geneset_directory, "/", sig_name, "_met_DOWN", ".gmt", sep="")) 
    }
}
...
if(nrow(all_genes) >= 5){
    ...
    sig_name <- paste(sep="_", met_name, "MetMap", "2022")
